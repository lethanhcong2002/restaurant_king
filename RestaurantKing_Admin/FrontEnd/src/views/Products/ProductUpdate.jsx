...
export default function ProductUpdate() {
    ...
    const [productCategory, setProductCategory] = useState(data.category || '0');
    const [productType, setProductType] = useState(data.type === true ? '1' : '0');
    const [selectedProducts, setSelectedProducts] = useState(data.selectedProduct || []);
    const [productImages, setProductImages] = useState(data.imageUrls || []);
    ...
    useEffect(() => {
        const fetchStorageData = async () => {
            try {
                ...
                (data.selectedProduct || []).forEach(product => {
                    initialQuantities[product.id] = product.quantity || 0;
                });

                setSelectedProductQuantities(initialQuantities);
    ...
    const saveProduct = async () => {
        const productData = {
            ...
            notes: productNotes,
            selectedProducts: productType === "0" ? [] : selectedProducts,
        ...
        try {
            const response = await axios.put(`http://localhost:5000/products/updateProduct/${id}`, productData, {
                headers: {
                    'Content-Type': 'application/json',
                },
            });
            alert('Sản phẩm đã cập nhật thêm thành công!');
            navigate(-1);
        } catch (error) {
                                                    ...
                                                    <table className="table table-striped mt-3">
                                                        ...
                                                        <tbody>
                                                            {selectedProducts.map((product, index) => (
                                                                <tr key={index}>
                                                                    <td>{getProductNameById(product.itemId)}</td>
                                                    ...
                                                    {components.map((component, index) => (
                                                        <tr key={index}>
                                                            ...
                                                            <td>
                                                                <CFormInput
                                                                    type="number"
                                                                    value={Number(component.quantity) || 0}
